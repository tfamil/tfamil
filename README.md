...
<p align="center">
  <img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&weight=500&pause=1000&center=true&width=700&lines=Full+Stack+%26+Mobile+Development;Node.js+%2F+TypeScript+%2F+PostgreSQL;Flutter+%2F+Dart+%2F+Kotlin+%2F+Android;React+%2F+Vite+%2F+JavaScript;REST+APIs+%2F+Testing+%2F+Git" alt="Typing SVG" />
</p>
...
| **Study Focus Guard — Webcam-Based Study Accountability Tool** | Built a self-contained, client-side study-accountability web app that uses real-time object detection on a live webcam feed to detect phone usage and trigger a full-screen intervention overlay. Implemented in-browser inference with TensorFlow.js and a pretrained COCO-SSD model (no backend, no data transmission), with a consecutive-frame confidence threshold to reduce false positives and a decay-based detection counter for stability. Designed a full-screen video overlay triggered on sustained detection, with auto-dismiss on video end, manual override, and a cooldown window to prevent re-trigger loops. Added a configurable sensitivity control (adjustable detection-frame threshold) and clear on/off/pause state management via the UI. Built entirely as a static, dependency-free single HTML file — no build step, no server, no external data storage — prioritizing local-only privacy by design. |
| **Study Focus Guard (Spidey-Themed GitHub Push Reminder)** | Python, Tkinter, Pillow, PowerShell, Windows Task Scheduler | **What I Built / Contributed:**
Built a transparent, always-on-top desktop reminder app in Python/Tkinter with custom canvas animations and Windows-native transparency. Used Pillow for image processing and alpha-channel background removal on assets. Automated daily scheduling via a PowerShell script that registers a Windows Task Scheduler job with missed-run recovery. |
